...
class DPR_mixcls(nn.Module):
    ...
    def encode_query(self, input_ids=None, attention_mask=None, token_type_ids=None):
        layers = self.q_encoder(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            return_dict=True,
        ).hidden_states[(13 - self.mix_layer) : 13]  # bert-base is 12 layer
    ...
    def forward(
        ...
        hn_input_ids=None,
        hn_attention_mask=None,
        hn_token_type_ids=None,
        labels=None,
        return_loss=True,
    ):
        ...
        if hn_input_ids is not None:
            hn_emb = self.encode_passage(
                hn_input_ids, hn_attention_mask, hn_token_type_ids
            )
...
class ScalarMixHead(nn.Module):
    def __init__(
        self, hidden_size=768, num_layers=12, proj=True, dropout=0.1, init_weight=0.65
    ):
        super().__init__()
        ...
        with torch.no_grad():
            ...
            if num_layers > 1:
                self.layer_w.data[-1] = math.log(
                    (init_weight * (num_layers - 1)) / (1.0 - init_weight)
                )

    def forward(self, cls_layers):  # list of [B,H]
        if self.layer_w.size(0) != 1:
            ...
            z = self.layer_norm(z)
            z = self.dropout(z)
            alpha = torch.ones(1, device=z.device, dtype=z.dtype)
            return z, alpha
